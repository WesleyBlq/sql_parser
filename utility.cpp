...
namespace oceanbase
{
  namespace common
  {
...
#if 0
    void print_rowkey(FILE *fd, string &rowkey)
    {
      char* pchar = rowkey.data();
    ...
    void print_root_table(FILE* fd, ObScanner &scanner)
    {
      ...
      while(OB_SUCCESS == scanner.next_cell())
      {
        if (OB_SUCCESS == scanner.get_cell(&cell, &is_row_changed))
        {
          ...
          fprintf(fd, "%.*s ", cell->column_name_.length(),  cell->column_name_.data());
    ...
    const char *print_string(const string &str)
    {
      static const int64_t BUFFER_SIZE = 16 * 1024;
      static __thread char buffer[BUFFER_SIZE] = "0x ";
      common::hex_to_str(str.data(), str.length(), &buffer[3], BUFFER_SIZE - 3);
      return buffer;
    }
      ...
      if (NULL != ci)
      {
        ...
        if (NULL == ext_info)
        {
          snprintf(buffer, BUFFER_SIZE, "table_id=%lu table_name=[%.*s] table_name_ptr=%p table_name_length=%d "
                  ...
                  "%s",
                  ci->table_id_, ci->table_name_.length(), ci->table_name_.data(), ci->table_name_.data(), ci->table_name_.length(),
                  rowkey_len, rowkey_str, ci->row_key_.data(), ci->row_key_.length(),
                  ci->column_id_, ci->column_name_.length(), ci->column_name_.data(), ci->column_name_.data(), ci->column_name_.length(),
                  print_obj(ci->value_));
        }
        else
        {
        ...
        for (; iter != scanner.row_end(); ++iter, ++row_num)
        {
          ...
          for (int64_t i = 0; i < cell_num; ++i)
          {
            used = snprintf(row_buffer + pos, row_bufsiz - pos, "column:[%ld][%.*s]:value[%s]%s",
                ci[i].column_id_, ci[i].column_name_.length(), ci[i].column_name_.data(),
    ...
    int64_t to_string<string>(const string &obj, char *buffer, const int64_t buffer_size)
    {
      ...
      int64_t ret = common::hex_to_str(obj.data(), obj.length(), &buffer[3], static_cast<int32_t>(buffer_size) - 3) + 3;
      if (ret >= buffer_size)
      {
        ret = buffer_size - 1;
      }
