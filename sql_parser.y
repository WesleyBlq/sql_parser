...
%token MEDIUMINT MEMORY MOD MODIFYTIME
%token NOT NUMERIC
...
%token DATABASES TABLE TABLES THEN TIME TIMESTAMP TINYINT TRAILING TRANSACTION TO PROCESSLIST
...
%type <node> explain_stmt explainable_stmt
...
%type <ival> opt_scope opt_drop_behavior
...
delete_stmt:
    ...
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_DELETE, 2, $3, $4);
    }
  ;
...
update_stmt:
    UPDATE relation_factor SET update_asgn_list opt_where
    {
      ...
      merge_nodes(assign_list, result->malloc_pool_, T_ASSIGN_LIST, $4);
      malloc_non_terminal_node($$, result->malloc_pool_, T_UPDATE, 3, $2, assign_list, $5);
    }
  ;
...
no_table_select:
    SELECT opt_hint opt_distinct select_expr_list opt_select_limit
    {
      ...
      malloc_non_terminal_node($$, result->malloc_pool_, T_SELECT, 14,
                              $3,             /* 1. distinct */
...
priv_type:
    ...
    {
      malloc_terminal_node($$, result->malloc_pool_, T_PRIV_TYPE);
      $$->value_ = T_PRIV_CREATE;
    }
    | CREATE USER
    {
      malloc_terminal_node($$, result->malloc_pool_, T_PRIV_TYPE);
      $$->value_ = T_PRIV_CREATE_USER;
    }
    | DELETE
    {
      malloc_terminal_node($$, result->malloc_pool_, T_PRIV_TYPE);
      $$->value_ = T_PRIV_DELETE;
    }
    | DROP
    {
      malloc_terminal_node($$, result->malloc_pool_, T_PRIV_TYPE);
    ...
    {
      malloc_terminal_node($$, result->malloc_pool_, T_PRIV_TYPE);
      $$->value_ = T_PRIV_GRANT_OPTION;
    }
    | INSERT
    {
      malloc_terminal_node($$, result->malloc_pool_, T_PRIV_TYPE);
      $$->value_ = T_PRIV_INSERT;
    }
    | UPDATE
    {
      malloc_terminal_node($$, result->malloc_pool_, T_PRIV_TYPE);
      $$->value_ = T_PRIV_UPDATE;
    }
    | SELECT
    {
      malloc_terminal_node($$, result->malloc_pool_, T_PRIV_TYPE);
      $$->value_ = T_PRIV_SELECT;
    }
    | REPLACE
    {
      malloc_terminal_node($$, result->malloc_pool_, T_PRIV_TYPE);
      $$->value_ = T_PRIV_REPLACE;
    }
;
opt_privilege:
    PRIVILEGES
